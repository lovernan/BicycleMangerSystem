...
<body style="background-color:#93DB70">
	<div class="container" >
		...
		<c:forEach items="${mybbsrent.allList }" var="rent">
			<div class="panel panel-default" style="margin-top: 10px; " >
				<div class="panel-heading" style="font-size: 25px;display:none" id="id">${rent.id }</div>
				<div class="panel-heading" style="font-size: 25px">${rent.name }</div>
				<div class="panel-body">
					<p>${rent.information }</p>
					<img style="width: 30%; height: 30%" src="${rent.image }">
					<p style="text-indent: 40em;">联系方式：${rent.tel }</p>
				</div>
		...
		<div style="width: 380px; margin: 0 auto; margin-top: 50px;">
			<ul class="pagination" style="text-align: center; margin-top: 10px;">
				<!-- 上一页 -->
				<!-- 判断当前页是否是第一页 -->
				<c:if test="${mybbsrent.currentPage==1 }">
				...
				<c:if test="${mybbsrent.currentPage!=1 }">
					<li><a
						href="${pageContext.request.contextPath }/mybbs?currentPage=${mybbs.currentPage-1}"
						aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
					</a></li>
				</c:if>

				<c:forEach begin="1" end="${mybbs.totalPage }" var="page">
					<!-- 判断当前页 -->
					<c:if test="${mybbsrent.currentPage==page }">
						<li class="active"><a href="javascript:void(0);">${page}</a></li>&nbsp;
					</c:if>
					<c:if test="${mybbsrent.currentPage!=page }">
						<li><a
							href="${pageContext.request.contextPath }/mybbs?currentPage=${page}">${page}</a></li>&nbsp;
					</c:if>

				</c:forEach>

				<!-- 判断当前页是否是最后一页 -->
				<c:if test="${mybbsrent.currentPage==mybbs.totalPage }">
					<li class="disabled"><a href="javascript:void(0);"
