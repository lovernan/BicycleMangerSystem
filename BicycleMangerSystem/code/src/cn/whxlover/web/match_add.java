...
public class match_add extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			BeanUtils.populate(match, params);
		} catch (IllegalAccessException | InvocationTargetException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		// 传数据到service层
		addSer match_addSer = new addSer();
		int issuccess = 0;
		try {
			issuccess = match_addSer.match_add(match);
		...
		response.sendRedirect("/Bicycle/matchlist2");
